...
const saveDetailInfo = detailURL => {
    ...
    return co(function*() {
        let html = yield Utils.load(detailURL.href);
        ...
        logger.error(`保存详情信息(${detailURL})失败:${error.message}`);
        return '';
    });
};
...
const saveCommunityInfo = url => {
    return co(function*() {
        ...
        let html = yield Utils.load(absoluteURL);
        ...
        logger.error(`保存小区信息(${url})失败:${error.message}`);
        return false;
    });
};
